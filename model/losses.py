...
class JaccardLoss(Loss):
    ...
    def __init__(self, class_weights=None, class_indexes=None, per_image=True, smooth=SMOOTH):
...
class DiceLoss(Loss):
    ...
    def __init__(self, beta=1, class_weights=None, class_indexes=None, per_image=True, smooth=SMOOTH):
...
class BinaryCELoss(Loss):
    ...
    def __call__(self, gt, pr):
        return F.binary_crossentropy(gt, pr)
...
class BinaryFocalLoss(Loss):
    ...
    def __call__(self, gt, pr):
        return F.binary_focal_loss(gt, pr, alpha=self.alpha, gamma=self.gamma)


class FocalTverskyLoss(Loss):
    r"""Creates a criterion to measure Focal Tversky loss:

    Args:
        alpha: parameter of the Tversky index.
        beta: paramenter of the Tversky index.
        gamma: parameter of the Focal Tversky Loss
        class_weights: Array (``np.array``) of class weights (``len(weights) = num_classes``).
        class_indexes: Optional integer or list of integers, classes to consider, if ``None`` all classes are used.
        per_image: If ``True`` loss is calculated for each image in batch and then averaged,
        else loss is calculated for the whole batch.
    ...
    def __init__(self, alpha=0.3, beta=0.7, gamma=4/3, class_weights=None, class_indexes=None, per_image=True, smooth=SMOOTH):
        super().__init__(name='dice_loss')
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
...
tversky_loss = FocalTverskyLoss(gamma = 1)
...
categorical_focal_tversky_loss = categorical_focal_loss + focal_tversky_loss
